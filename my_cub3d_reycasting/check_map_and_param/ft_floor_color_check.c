...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_floor_color_check.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: eshakita <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/31 10:59:19 by eshakita          #+#    #+#             */
/*   Updated: 2021/04/06 14:53:39 by eshakita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_digit_color_pars(t_tar *tar, char *buf, int *i, int flag)
{
	...
	if (int_tmp_color < 0 || int_tmp_color > 255)
		ft_free_close(buf);
	if (flag == 1)
		tar->floor = tar->floor | (int_tmp_color << 16);
	if (flag == 2)
		tar->floor = tar->floor | (int_tmp_color << 8);
	if (flag == 3)
		tar->floor = tar->floor | int_tmp_color;
}
				...
				t_map_flag_args *map_flag_args)
{
	*i += 2;
	...
	if (buf[*i] != '\n')
		ft_free_close(buf);
	if (map_flag_args->floor == 1)
		ft_free_close(buf);
	map_flag_args->floor = 1;
}
